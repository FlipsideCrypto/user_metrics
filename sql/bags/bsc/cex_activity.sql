 WITH cex_addresses AS (
   ...
   bnb_from_cex AS (
  SELECT NULL as token_contract, 'bnb' as token_symbol,
   COUNT(*) as n_withdrawals, SUM(AMOUNT) as wdraw_token_volume, SUM(AMOUNT_USD) as wdraw_usd_volume,
  bnb_TO_ADDRESS as user_address, cex_addresses.LABEL as exchange_name
  FROM bsc.CORE.EZ_bnb_TRANSFERS LEFT JOIN cex_addresses ON bnb_FROM_ADDRESS = cex_addresses.ADDRESS
  WHERE BLOCK_TIMESTAMP >= DATEADD('day', -90, CURRENT_DATE()) AND
        bnb_FROM_ADDRESS IN (SELECT ADDRESS FROM cex_addresses) AND
        bnb_TO_ADDRESS NOT IN (SELECT ADDRESS FROM cex_addresses)
...
from_cex AS (
  ...
  FROM bsc.CORE.EZ_TOKEN_TRANSFERS LEFT JOIN cex_addresses ON FROM_ADDRESS = cex_addresses.ADDRESS
  WHERE BLOCK_TIMESTAMP >= DATEADD('day', -90, CURRENT_DATE()) AND
...
to_cex AS (
  ...
  FROM bsc.CORE.EZ_TOKEN_TRANSFERS LEFT JOIN cex_addresses ON TO_ADDRESS = cex_addresses.ADDRESS
  WHERE BLOCK_TIMESTAMP >= DATEADD('day', -90, CURRENT_DATE()) AND
...
GROUP BY user_address, exchange_name, token_contract, token_symbol
  ...
  SELECT NULL as token_contract, 'bnb' as token_symbol,
   COUNT(*) as n_deposits, SUM(AMOUNT) as dep_token_volume, SUM(AMOUNT_USD) as dep_usd_volume,
  bnb_FROM_ADDRESS as user_address, cex_addresses.LABEL as exchange_name
  FROM bsc.CORE.EZ_bnb_TRANSFERS LEFT JOIN cex_addresses ON bnb_TO_ADDRESS = cex_addresses.ADDRESS
  WHERE BLOCK_TIMESTAMP >= DATEADD('day', -90, CURRENT_DATE()) AND
        bnb_FROM_ADDRESS NOT IN (SELECT ADDRESS FROM cex_addresses) AND
        bnb_TO_ADDRESS IN (SELECT ADDRESS FROM cex_addresses)
GROUP BY user_address, exchange_name, token_contract, token_symbol
   )
...
SELECT user_address, exchange_name, token_contract, token_symbol,
 ...
 UNION (SELECT user_address, exchange_name, token_contract, token_symbol,
   n_deposits, n_withdrawals, dep_token_volume, dep_usd_volume, wdraw_token_volume, wdraw_usd_volume 
   FROM bnb_from_cex NATURAL FULL OUTER JOIN bnb_to_cex)