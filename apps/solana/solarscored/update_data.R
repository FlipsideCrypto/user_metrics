# this file will be updated to use ShroomDK - it is not currently useable by the public without replacing
# the function QuerySnowflake with a package from the R ShroomDK package
...
# fd.melt <- melt.data.table(full.data[staked_sol < 10000000 & n_bridge < 10000, list(user_address, total_score, n_txn, n_programs, nft_volume, staked_sol, n_bridge, n_votes)],
#                 id.vars = c("user_address", "total_score"))
# fd.melt2 <- melt.data.table(fd.melt[, list(median = median(value),
#                                            mean = mean(value),
#                                            max = max(value)), by = list(total_score, category = variable)],
#                             id.vars = c("total_score","category"))
# ggplot(fd.melt2,
#        aes(x = total_score, y = value, color = variable, shape = variable)) + 
#   geom_point() + 
#   facet_wrap(~category, scales = "free") +
#   theme_flipside_roboto() +
#   scale_color_manual(values = c("black", "#a288a6", "#bb9bb0")) 
